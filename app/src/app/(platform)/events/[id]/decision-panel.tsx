...
export function DecisionPanel({ event }: DecisionPanelProps) {
  ...
  if (confirmed) {
    return (
      <div className="flex flex-col items-center justify-center gap-5 rounded-2xl bg-[#1a0f2e] p-8 ring-1 ring-emerald-500/30 text-center">
        ...
        <div>
          <p className="font-[family-name:var(--font-anton)] text-xl text-white">
            Previsão registrada
          </p>
          <p className="mt-2 text-sm text-white/60">
            Posição <strong className="text-white">{side}</strong> de{" "}
            <strong className="text-white">R$ {numValue.toFixed(2).replace(".", ",")}</strong> registrada com sucesso.
          </p>
        ...
        <button
          onClick={() => { setConfirmed(false); setSide(null); setRawValue(""); }}
          className="text-xs font-semibold text-[var(--oraklo-color-primary-glow)] hover:underline"
        >
          Registrar nova previsão
        </button>
      </div>
    );
  }
    ...
    <div className="sticky top-6 space-y-4">
      ...
      <div className="rounded-2xl bg-[#1a0f2e] p-5 ring-1 ring-white/10">
        ...
        <div className="mt-4 grid grid-cols-2 gap-3">
          ...
          <button
            onClick={() => setSide("SIM")}
            className={`group flex flex-col gap-2 rounded-xl p-4 ring-1 transition-all ${
              side === "SIM"
      ...
      <div className="rounded-2xl bg-[#1a0f2e] p-5 ring-1 ring-white/10">
        ...
        <button
          ...
          className="mt-4 w-full rounded-xl bg-amber-500 py-3 text-sm font-bold uppercase tracking-[0.06em] text-white transition-all hover:bg-amber-400 disabled:cursor-not-allowed disabled:opacity-30"
        >
          Validar minha previsão
        </button>
