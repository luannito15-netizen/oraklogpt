...
export function EventHotHero({ event, onOpen }: EventHotHeroProps) {
  ...
  const cta = onOpen ? (
    <button
      ...
      className="bg-[var(--oraklo-color-primary)] hover:bg-violet-500 text-white font-bold rounded-xl px-6 py-2.5 text-sm transition-all hover:scale-[1.02] active:scale-[0.98]"
    >
      PARTICIPAR &rarr;
    </button>
  ) : (
    <Link
      ...
      className="bg-[var(--oraklo-color-primary)] hover:bg-violet-500 text-white font-bold rounded-xl px-6 py-2.5 text-sm transition-all hover:scale-[1.02] active:scale-[0.98] inline-block"
    >
      PARTICIPAR &rarr;
    </Link>
  )
  ...
  return (
    <div className="relative overflow-hidden bg-gradient-to-br from-[#1e0a3c] via-[#1a0f2e] to-[#0d0717] ring-1 ring-white/10 rounded-2xl p-6">
      ...
      {event.imageUrl && (
        <div className="absolute right-0 top-0 bottom-0 w-1/3 hidden lg:block overflow-hidden rounded-r-2xl">
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-r from-[#1e0a3c] to-transparent" />
        </div>
      )}
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-2">
          <span className="bg-red-500/20 text-red-400 ring-1 ring-red-500/30 rounded-full px-2.5 py-0.5 text-[10px] font-black uppercase animate-pulse">
            HOT
          </span>
      ...
      {/* Title */}
      <p className="text-2xl font-black text-white leading-snug mt-4 max-w-2xl line-clamp-2">
        {event.title}
      </p>
      ...
      <div className="mt-5 flex gap-3">
        <div className="flex flex-1 flex-col gap-1 rounded-xl bg-emerald-500/15 py-4 px-5 ring-1 ring-emerald-500/25">
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm font-bold text-white">{cotacao(event.simPercent)}</span>
        ...
        <div className="flex flex-1 flex-col gap-1 rounded-xl bg-orange-500/15 py-4 px-5 ring-1 ring-orange-500/25">
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm font-bold text-white">{cotacao(nao)}</span>
